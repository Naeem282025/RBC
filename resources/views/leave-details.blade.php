								...
								<div class="append mr-auto my-0 my-md-0 mr-auto">
									<ul class="list-group list-group-horizontal-md mr-auto">
										...
										<li class="mr-1 active"><a
											class="text-white btn-ctm-space" href="leave-details.html"><span
				...
				<div class="row">
					...
					<div class="col-xl-9 col-lg-8 col-md-12">
						...
						<div class="col-md-12">
							<div class="card ctm-border-radius shadow-sm">
								<div class="card-header">
									<h4 class="card-title mb-0">Total Leave Details</h4>
								</div>
								<div class="card-body">
											...
											<table class="table custom-table mb-0 table-hover">
												...
												<tbody>
													<tr>
														...
														<td>{{ $totalEmployees ?? 0 }}</td>
														<td>{{ $sickLeave > 0 ? $sickLeave : 'N/A' }}</td>
														<td>{{ $parentalLeave > 0 ? $parentalLeave : 'N/A' }}</td>
														<td>{{ $workingFromHome > 0 ? $workingFromHome : 'N/A' }}</td>
													</tr>
						...
						<div class="col-md-12">
							<div class="card ctm-border-radius shadow-sm">
								<div class="card-header">
									<h4 class="card-title mb-0">Leave Details List</h4>
								</div>
								<div class="card-body">
									<div class="employee-office-table">
										<div class="table-responsive">
												...
												<tbody>
													@forelse($leaveList as $index => $leave)
														<tr>
															...
															<td>{{ $days[$index] ?? 'N/A' }}</td> <!-- Ensuring $days[$index] exists -->
															<td>{{ $leave->status }}</td>
															...
															<td>
																<form action="{{ url('/view') }}" method="post">
																	@csrf
																	<input type="hidden" name="employeeId" value="{{ $leave->employee->id }}">
																	<button type="submit" class="btn btn-theme ctm-border-radius text-white btn-sm">View</button>
																</form>
															</td>
															<!-- Approve Button -->
															<td>
																<form action="{{ route('approve.leave') }}" method="post">
																	@csrf
																	<input type="hidden" name="leaveId" value="{{ $leave->id }}">
																	<button type="submit" class="btn btn-success ctm-border-radius text-white btn-sm">Approve</button>
																</form>
															</td>
															<!-- Reject Button -->
															<td>
																<form action="{{ route('reject.leave') }}" method="post">
																	@csrf
																	<input type="hidden" name="leaveId" value="{{ $leave->id }}">
																	<button type="submit" class="btn btn-danger ctm-border-radius text-white btn-sm">Reject</button>
																</form>
															</td>
													...
													@empty
														<tr>
															<td colspan="9" class="text-center">No leave records found</td> <!-- Updated colspan to 9 -->
														</tr>
													@endforelse
												</tbody>
